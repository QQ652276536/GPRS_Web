...
public interface DeviceInfoRepository extends JpaRepository<DeviceInfo, Integer>
{
    ...
    @Query("update DeviceInfo device set device.m_state = 1,device.m_lat = :lat,device.m_lot = :lot,device.m_height = :height,device" +
            ".m_updateTime = CURRENT_TIMESTAMP" + " where " + "device.m_name = " + ":name")
    ...
    @Modifying(clearAutomatically = true)
    @Query("update DeviceInfo device set device.m_state = 1,device.m_lat = :lat,device.m_lot = :lot,device.m_height = :height,device" +
            ".m_updateTime = CURRENT_TIMESTAMP" + " where " + "device.m_id = " + ":id")
    int UpdateDeviceById(@Param("id") int id, @Param("lat") double lat, @Param("lot") double lot, @Param("height") double height);
}
