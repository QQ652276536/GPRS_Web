...
public interface LocationInfoRepository extends JpaRepository<LocationInfo, Integer>
{
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM locationinfo where device_id = ? and DATE_SUB(CURDATE(), INTERVAL ? DAY) <=" +
            " date(create_time) and lat > 0 and lot > 0 GROUP BY DATE(create_time) ORDER BY create_time DESC")
