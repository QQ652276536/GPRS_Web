...
public class FileContentEvent_GPRS extends ApplicationEvent {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FileContentEvent_GPRS.class);
    ...
    public FileContentEvent_GPRS(FileData fileData) {
        ...
        LOGGER.info(String.format("线程%s执行...", readFileThread.getId()));
    }
    ...
    class ReadFileThread extends Thread {
        ...
        private void ReadFile() {
            ...
            try {
                ...
                lineCount = array.length;
                LOGGER.info(String.format("本次共读取(过滤了空行):%s", lineCount));
                //文件内容有变动
                if (lineCount != LINECOUNT) {
                    ...
                    if (lat != 0.0 && lot != 0.0) {
                        ...
                        LOGGER.info(String.format("将本次数据%s更新至MySQL数据库...", locationInfo.toString()));
                    } else {
