...
public class FileContentEvent_YX {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FileContentEvent_YX.class);
    ...
    public void ReadFromEamilFile(String path) {
        ...
        try {
            ...
            LOGGER.debug(String.format("文件内容(HX):" + hexStr));
            ...
            LOGGER.debug(String.format("消息ID:%s消息流水:%s纬度:%s经度:%s", idStr, hexMessageFlow, latNumXxx, lotNumXxx));
            if (1 > 0) {
                return;
            }
            ...
            if (lat != 0.0 && lot != 0.0) {
                ...
                LOGGER.debug(String.format("将本次数据%s更新至MySQL数据库", locationInfo.toString()));
...
//                String locationResult = new SocketHttp().SendPost(_webIP, _webPort, "/GPRS_Web/LocationInfo/InsertList", locationStr);
                ...
                LOGGER.debug(String.format("汇报铱星设备位置,返回:%s", locationResult));
            } else {
                LOGGER.error("本次数据有错误,禁止更新至数据库");
            }
        } catch (Exception e) {
    ...
    class ReadFileThread extends Thread {
        ...
        public void ReadFile(String path) {
            ...
            try {
                ...
                LOGGER.info("本次共读取(过滤了空行):" + lineCount);
                //文件内容有变动
        ...
        public void ReadAllFile(String path) {
            ...
            try {
                ...
                if (locationInfoList.size() > 0) {
                    LOGGER.debug(String.format("本次共读取(过滤了空行):%d条数据,新增%d条正确数据", lineCount, locationInfoList.size()));
